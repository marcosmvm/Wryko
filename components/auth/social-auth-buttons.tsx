...
export function SocialAuthButtons({ redirectTo = '/dashboard', termsAccepted = true }: SocialAuthButtonsProps) {
  const [isGoogleLoading, setIsGoogleLoading] = useState(false)
  const [isGitHubLoading, setIsGitHubLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const isAnyLoading = isGoogleLoading || isGitHubLoading
  const isDisabled = isAnyLoading || !termsAccepted
    ...
    <div className="space-y-3">
      ...
      {!termsAccepted && (
        <p className="text-xs text-muted-foreground text-center">
          Please accept the Terms of Service and Privacy Policy above to use social login
        </p>
      )}
